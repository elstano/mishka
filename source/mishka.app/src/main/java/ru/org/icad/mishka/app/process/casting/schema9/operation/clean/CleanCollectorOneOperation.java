package ru.org.icad.mishka.app.process.casting.schema9.operation.clean;
...
public class CleanCollectorOneOperation extends Operation {
    ...
    public void activate() {
        ...
        if (isNeedClean()) {
            ...
            LOGGER.debug("Result - castingUnitId: " + schema.getSchemaConfiguration().getCastingUnitId()
                    + ", Operation type: CleanCollectorOneOperation startDate: "
                    + TimeUtil.convertTimeToString(getActivationDate().getTime()) + ", cleanTime: " + time / 60 / 1000);
        }

        Operation operation = schema.getOperationMap().get(OperationName.PREPARE_COLLECTOR_ONE);
    ...
    private boolean isNeedClean() {
        ...
        if(periodicOperation.getCastingUnitCollector().getId() != schema.getSchemaConfiguration().getCastingUnitCollectorIds()[0]) {
            return false;
        }
