...
public class CastCmThreeCollectorTwoOperation extends Operation {
    ...
    public void activate() {
        ...
        if ("flush".equals(customerOrder.getId())) {
            ...
            Query castingSpeedQuery = em.createNativeQuery("SELECT * from CASTING_SPEED cs where cs.MOULD_ID = " + schema.getSchemaConfiguration().getMouldIds()[2]
        ...
        Query castingUnitRepairsQuery = em.createNativeQuery("SELECT * from CU_REPAIR cur where cur.CAST_MACH_ID = "
                +  schema.getSchemaConfiguration().getCastingUnitCastingMachineIds()[2] , CastingUnitRepair.class);
        ...
        if(castingUnitRepairs != null) {
            for(Object object : castingUnitRepairs) {
                ...
                LOGGER.debug("Result - castingUnitId: " + schema.getSchemaConfiguration().getCastingUnitId()
                        + ", Operation type: CastCmThreeCollectorTwoOperation Repair, customer order id: " + customerOrder.getId());
            }
        }
