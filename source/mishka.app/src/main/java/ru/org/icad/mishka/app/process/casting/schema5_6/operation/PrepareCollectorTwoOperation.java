...
public class PrepareCollectorTwoOperation  extends Operation {
    private static final Logger LOGGER = LoggerFactory.getLogger(PrepareCollectorTwoOperation.class);

    private final Schema schema;

    public PrepareCollectorTwoOperation(Schema schema) {
        this.schema = schema;
    }
    ...
    public void activate() {
        ...
        Query castingUnitRepairsQuery = em.createNativeQuery("SELECT * from CU_REPAIR cur where cur.COLLE_ID = "
                + schema.getSchemaConfiguration().getCastingUnitCollectorIds()[1], CastingUnitRepair.class);
        ...
        if(castingUnitRepairs != null) {
            for(Object object : castingUnitRepairs) {
                ...
                LOGGER.debug("Result - castingUnitId: " + schema.getSchemaConfiguration().getCastingUnitId()
                        + ", Operation type: PrepareCollectorTwoOperation Repair");
            }
        }
        ...
        LOGGER.debug("Result - castingUnitId: " + schemaConfiguration.getCastingUnitId()
                + ", Operation type: PrepareCollectorTwoOperation startDate: "
